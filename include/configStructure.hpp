...
struct ServerConfig
{
	...
	std::map<int, std::string>				error_page;				// 선택 필드, 입력값 없으면 size() == 0
	std::string								server_name;			// 선택 필드, 입력값 없으면 ""
...
struct LocationConfig
{
	// std::string					location_path;	// key로 이동 (std::map<std::string, LocationConfig>	locations)
	std::map<Method, bool>		allow_method;	// 필수 필드, 입력값 없으면 default를 MethodType enum으로 초기화
	bool						autoindex;		// 필수 필드, 입력값 없으면 default를 false -> "off"를 의미
